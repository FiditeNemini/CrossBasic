/*
  ...
  HTMLtoMarkdown.rs
  CrossBasic Plugin: HTMLtoMarkdown
  ...
  Build (Windows): rustc --crate-type=cdylib HTMLtoMarkdown.rs -o libHTMLtoMarkdown.dll
  Build (macOS/Linux): rustc --crate-type=cdylib HTMLtoMarkdown.rs -o libHTMLtoMarkdown.dylib/so
...
fn html_to_markdown_impl(html: &str) -> String {
    ...
    // --- Table support ---
    // Remove <table>, <thead>, <tbody>, <tfoot> tags
    let re_table_start = Regex::new(r"(?i)<table.*?>").unwrap();
    md = re_table_start.replace_all(&md, "").to_string();
    let re_table_end = Regex::new(r"(?i)</table>").unwrap();
    md = re_table_end.replace_all(&md, "\n").to_string();
    let re_thead = Regex::new(r"(?i)</?thead>").unwrap();
    md = re_thead.replace_all(&md, "").to_string();
    let re_tbody = Regex::new(r"(?i)</?tbody>").unwrap();
    md = re_tbody.replace_all(&md, "").to_string();
    let re_tfoot = Regex::new(r"(?i)</?tfoot>").unwrap();
    md = re_tfoot.replace_all(&md, "").to_string();

    // Convert <tr> to start a new line
    let re_tr_open = Regex::new(r"(?i)<tr.*?>").unwrap();
    md = re_tr_open.replace_all(&md, "\n").to_string();
    let re_tr_close = Regex::new(r"(?i)</tr>").unwrap();
    md = re_tr_close.replace_all(&md, "|\n").to_string();

    // Convert <th> and <td> cells
    let re_th = Regex::new(r"(?i)<th>(.*?)</th>").unwrap();
    md = re_th.replace_all(&md, "| **$1** ").to_string();
    let re_td = Regex::new(r"(?i)<td>(.*?)</td>").unwrap();
    md = re_td.replace_all(&md, "| $1 ").to_string();

    // Remove any remaining <th> or <td> tags (shouldn't be necessary but for safety)
    let re_cell_tags = Regex::new(r"(?i)</?(th|td).*?>").unwrap();
    md = re_cell_tags.replace_all(&md, "").to_string();

    // --- End table support ---

    // Replace image tags: <img src="URL" alt="ALT"> or variations thereof.
    let re_img = Regex::new(
        r#"(?i)<img\s+[^>]*src="([^"]+)"[^>]*alt="([^"]*)"[^>]*>"#
    ).unwrap();
    md = re_img.replace_all(&md, "![$2]($1)").to_string();
    let re_img_alt_first = Regex::new(
        r#"(?i)<img\s+[^>]*alt="([^"]*)"[^>]*src="([^"]+)"[^>]*>"#
    ).unwrap();
    md = re_img_alt_first.replace_all(&md, "![$1]($2)").to_string();

    // Replace anchor tags: <a href="URL">TEXT</a> -> [TEXT](URL)
    let re_link = Regex::new(
        r#"(?i)<a\s+[^>]*href="([^"]+)"[^>]*>(.*?)</a>"#
    ).unwrap();
    md = re_link.replace_all(&md, "[$2]($1)").to_string();
...
pub extern "C" fn urltomarkdown(url: *const c_char) -> *const c_char {
    ...
    // Decode bytes as either UTF-8 or fallback to ISO-8859-1 (Latin1) if UTF-8 fails.
    let html_str = match String::from_utf8(html_content.clone()) {
        Ok(s) => s,
        Err(_) => {
            html_content.iter().map(|&b| b as char).collect()
        }
    };
